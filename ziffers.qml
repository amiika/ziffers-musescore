...
MuseScore {
    ...
    RowLayout { // Output type
        ...
        ComboBox {
            ...
            model: ListModel {
                ...
                ListElement {
                    text: "Array with lines";
                    oName: 2
                }
                ListElement {
                    text: "Array without lines";
                    oName: 3
                }
                ListElement {
                    text: "Tracker array";
    ...
    property var _typeof: function (obj) {
        ...
        for (var staff = startStaff; staff <= endStaff; staff++) {
            ...
            for (var voice = 0; voice < 4; voice++) {
                ...
                while ((measure && fullScore) || (measure && cursor.tick < endTick)) {
                    ...
                    while (segment) {
                        ...
                        if (sType == "BeginBarLine") {
                            ...
                            if (includeRepeats.checked && !(outputType.key == 4 || outputType.key == 5))
                                measureString += "[: "
                                startRepeatCount += 1;
                            ...
                            if (el.type == Element.CHORD) {
                                        ...
                                        if (k == 0) { // Only for first occurance for chords
                                            ...
                                            if (!tieForward) {
                                                if (durationList.key != 3 && (lastLength != currentLength || (outputType.key != 1 && firstInMeasure) || tieBack)) {
                                                    if (durationList.key == 0 && currentLength != undefined)
                                                        measureString += currentLength + " ";
                                                    if (durationList.key == 2 || currentLength == undefined)
                                                        measureString += parseFloat((tieBack ? ((lastTicks / 1920) + (currentTicks / 1920)) : (currentTicks / 1920)).toFixed(4)) + " ";
                                                }
                                            }

                                            // Repeated durations
                                            if (!tieForward && durationList.key == 1)
                                                measureString += currentLength;
                                ...
                                if (durationList.key != 3 && (lastLength != currentLength || (outputType.key != 1 && firstInMeasure))) {
                                    if (durationList.key == 0 && currentLength != undefined)
                                        measureString += currentLength + " ";
                                    if (durationList.key == 2 || currentLength == undefined)
                                        measureString += parseFloat((el.duration.ticks / 1920).toFixed(4)) + " ";
                                }

                                if (durationList.key == 1)
                                    measureString += currentLength;
                ...
                if (elementsInVoice && voiceRows.length > 0) {
                    if (outputType.key == 0)
                        voiceRows = voiceRows.join("| \\\n| ");
                    if (outputType.key == 1)
                        voiceRows = voiceRows.join(" ");
                    if (outputType.key == 3 || outputType.key == 4 || outputType.key == 5)
                        voiceRows = voiceRows.concat.apply([], voiceRows);
